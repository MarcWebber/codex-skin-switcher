...
- The plugin registers its minimal Watcher when the MCP server starts. The Watcher only relaunches Codex after an explicit non-native `set_skin` request while CDP is unavailable, and consumes that request once. Later normal quits stay closed.
- Report the returned message. When `cdpReady` is false, ask the user to quit Codex normally once; the Watcher will reopen it with the local CDP parameter. The toolbar waits briefly for its local binding and refreshes automatically. Never restart or kill Codex from the agent turn.
