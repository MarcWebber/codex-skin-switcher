...
当前安装、切换、创建、市场下载、确认删除和原生恢复链路已经可以面向 macOS 用户推广。公开介绍时应同时说明三项边界：目前是 **macOS only**；只维护当前 Codex 页面结构；角色或第三方素材不因本项目的 MIT License 获得再授权。两个 GitHub 仓库的 `main` 已启用分支保护并要求通过 Pull Request；接入 CI 后再增加必需状态检查。
